...
class ContinentalContent extends DataExtension {
	...
	private static $provider = 'MaxMind';
	...
	{
		if(count(self::$debug_messages)) {
			return implode('<br><br>', self::$debug_messages);
		}
		return false;
	}
	...
	public static function GetContinents(){
		...
		foreach(Config::inst()->get('ContinentalContent', 'continents') as $key => $filter)
			$arrRet[self::UpdateContinentExtensionName($key)] = $filter;
		return $arrRet;
	}
	...
	public static function CurrentContinent(){
		...
		if(!self::IsViewingThroughProxy() && Session::get('SESSION_MAP_LOCATION')){
			...
			foreach(self::GetContinents() as $key => $filter) {
				if($filter['Country'] == $forwarder->Continent){
					self::$current_continent = $key;
					ContinentalContent::add_debug_message('Forwarder Matched : ' . $forwarder->Continent);
					break;
				}
			}
		}
			...
			foreach(self::GetContinents() as $key => $filter){
				$cityMatched = -1;
				$subDivisionMatched = -1;
				$countryMatched = -1;

				if(isset($filter['City'])) {
					$cityMatched = 0;
					$cities = is_array($filter['City']) ? $filter['City'] : array($filter['City']);
					$intersectCities = array_intersect($cities, $location->CityNames);
					if(count($intersectCities)) {
						$cityMatched = 1;
					}
				}

				if(isset($filter['SubDivision'])) {
					$subDivisionMatched = 0;
					$subdivisions = is_array($filter['SubDivision']) ? $filter['SubDivision'] : array($filter['SubDivision']);
					$intersectSubdivisions = array_intersect($subdivisions, $location->SubDivisions);
					if(count($intersectSubdivisions)) {
						$subDivisionMatched = 1;
					}
				}
				...
				if(isset($filter['Country'])) {
					$countryMatched = 0;
					$countries = is_array($filter['Country']) ? $filter['Country'] : array($filter['Country']);
					if(in_array($location->Country, $countries)){
						$countryMatched = 1;
					}
				}

				if($cityMatched != 0 && $subDivisionMatched != 0 && $countryMatched != 0) {
					self::$current_continent = $key;
					ContinentalContent::add_debug_message('Matched Location : ' . $key);
					break;
				}
			}
		}
